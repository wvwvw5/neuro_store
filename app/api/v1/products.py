...
async def get_product_plans(
    ...
    try:
        ...
        logger.info("Product plans fetched successfully", product_id=product_id, plans_count=len(plans))
        return plans
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error("Error fetching product plans", product_id=product_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
