...
def topup_balance(
    ...
    try:
        logger.info(
            f"Попытка пополнения баланса для пользователя {current_user.email}, сумма: {request.amount}"
        )

        # Проверяем данные карты (заглушка для демо)
        if not request.card_number or len(request.card_number) < 16:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Неверный номер карты"
            )
        ...
        if not request.cvv or len(request.cvv) != 3:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Неверный CVV код"
            )
        ...
        logger.info(
            f"Код верификации для пользователя {current_user.email}: {verification_code}"
        )
...
def verify_payment(
    ...
    try:
        logger.info(
            f"Попытка верификации платежа для пользователя {current_user.email}"
        )
        ...
        logger.info(
            f"Баланс пользователя {current_user.email} пополнен на {payment.amount}"
        )
...
def get_balance(
    ...
    return {"balance": current_user.balance, "currency": "RUB"}
...
def get_topup_statistics(
    ...
    admin_role = (
        db.query(UserRole)
        .filter(
            UserRole.user_id == current_user.id, UserRole.role_id == 1  # ID роли admin
        )
        .first()
    )
    ...
    total_topups = (
        ...
        .filter(Payment.status == "completed", Payment.payment_method == "card")
        .scalar()
        or 0
    )
    ...
    monthly_stats = (
        ...
        .order_by(func.date_trunc("month", Payment.payment_date).desc())
        .limit(12)
        .all()
    )
